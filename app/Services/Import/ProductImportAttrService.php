...
class ProductImportAttrService
{
    public function import(int $limit = 0)
    {
        $sql = file_get_contents(storage_path('legacy/productsAttr.sql'));
        ...
        foreach ($rows as $i => $row) {
            ...
            $product->new_model = $data[1];
            $product->new_leather_color = $data[3];
            $product->new_leather = $data[2];
            $product->created_at = $data[4];
            $product->updated_at = $data[5];
