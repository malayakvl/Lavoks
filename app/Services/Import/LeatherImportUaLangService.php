...
use App\Models\LeatherTranslation;
use Illuminate\Support\Str;

class LeatherImportUaLangService
{
    ...
    {
        $sql = file_get_contents(storage_path('legacy/lazer_types.sql'));
        ...
        foreach ($rows as $i => $row) {
            ...
            // Debug first 3 rows
            if ($i < 3) {
                \Log::info("Leather Row {$i}: " . json_encode($data));
            }
            $oldLeatherId = (int)($data[0] ?? 0);
            $leather = Leather::where('old_id', $oldLeatherId)->first();

            if (!$oldLeatherId) {
                continue;
            }

            if (!$leather || !$leather->id) {
                \Log::error("Failed to find leather with old_id: {$oldLeatherId}");
                continue;
            }

            LeatherTranslation::updateOrCreate(
                [
